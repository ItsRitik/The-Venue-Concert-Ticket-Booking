    ...
    products
      ? products.map((product, index) => (
          <div className="user_product_block" key={`${product._id}${index}`}>
            <div className="item">
              <p className="text-xl sm:text-3xl font-bold mb-2">{product.Type}</p>
            </div>
            <div className="item">
            <h4 className="text-sm text-center sm:text-xl font-semibold">No.of Tickets</h4>
              <div className="flex justify-center items-center">
                ...
                <button
  className="border rounded bg-blue-500 text-white px-2 sm:px-8"
...
<button
  className="border rounded bg-red-500 text-white px-2"
